...
>我的迷思： 以為 BDD == 測試介面流程，BDD == 整合測試 ，TDD == 單元測試，而且這是不可動搖的。

然而BDD是TDD的進化，重點著重在著寫測試案例時，群組這些測試的是需求的情境，但驗証這些情境是否正確的，還是TDD，所以TDD的精神並沒有不見(例如：在某個情境中還是要依靠驗証方法的正確性來確認需求是否完成)。  

事實上TDD及BDD只是在呈述測試時的角度不同。就算是單純共用的函數，也可以使用BDD的呈述來表達當初設計時程式時想要解決的需求情境。  
